"""
Taken from https://github.com/grammarly/gector/blob/master/utils/preprocess_data.py

Might include formatting and other slight modifications.
"""
...
from gector_lightning.utils.helpers import (
    SEQ_DELIMITERS,
    START_TOKEN,
...
class BrokenSentencePairException(Exception):
    pass
...
def is_sent_ok(sent, delimeters=SEQ_DELIMITERS):
...
def check_casetype(source_token, target_token):
    ...
    elif capitalize(source_token) == target_token:
        return "$TRANSFORM_CASE_CAPITAL"
    elif source_token.upper() == target_token:
        return "$TRANSFORM_CASE_UPPER"
    elif (
        capitalize(source_token) == target_token[1:]
        and source_token[0] == target_token[0]
    ):
        return "$TRANSFORM_CASE_CAPITAL_1"
    elif (
        source_token[:-1].upper() == target_token[:-1]
        and source_token[-1] == target_token[-1]
    ):
...
def align_sequences(source_sent, target_sent, return_type="tagged"):
    # check if sent is OK
    ...
    for diff in diffs:
        ...
        if tag == "equal":
            ...
            # Skip if source/target is too long
            if len(source_part) > 500 or len(target_part) > 500:
                raise BrokenSentencePairException
            ...
            _, alignments = perfect_align(
                source_part, target_part, insertions_allowed=0
            )
            for alignment in alignments:
                new_shift = alignment[2][0]
                edits = convert_alignments_into_edits(
                    alignment, shift_idx=i1 + new_shift
                )
                all_edits.extend(edits)

    if return_type == "edits":
        return all_edits
...
def convert_data_from_raw_files(source_data, target_data, output_file, chunk_size):
    tagged = []
    ...
    for source_sent, target_sent in tqdm(zip(source_data, target_data)):
        ...
        except BrokenSentencePairException:
            continue
        except Exception:
            aligned_sent = align_sequences(source_sent, target_sent)
...
def convert_tagged_line(line, delimeters=SEQ_DELIMITERS):
    ...
    source_tokens = [x.split(label_del)[0] for x in line.split(delimeters["tokens"])][
        1:
    ]
    labels = [
