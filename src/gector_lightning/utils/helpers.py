"""Taken from https://github.com/grammarly/gector/blob/master/utils/helpers.py

Might include formatting and other slight modifications.
"""
import os

from gector_lightning.constants import DATA_DIR_PATH
...
def capitalize(text: str) -> str:
    """Capitalize only the first letter and leaves the rest unchanged."""
    return text[0].upper() + text[1:] if len(text) else ""


def get_verb_form_dicts():
    path_to_dict = DATA_DIR_PATH / "verb-form-vocab.txt"
    encode, decode = {}, {}
...
def convert_using_case(token, smart_action):
    ...
    if smart_action.endswith("LOWER"):
        ...
        return token[0] + capitalize(token)
...
def read_parallel_lines(fn1, fn2):
    ...
    return remove_empty_lines(lines1, lines2)


def remove_empty_lines(lines1, lines2):
